...
export function generateEmployees () {
  return new Promise((resolve, reject) => {
    // Creates initial list of employees and managers
    const employees = []
    const managers = []
    ...
    axios.get(`https://www.humanity.com/api/v2/employees?access_token=${process.env.API_KEY}`)
      .then((response) => {
        ...
        response.data.data.forEach(employee => {
          ...
          // Checks if the employee with the manager title has a name in the list provided (Names must be an exact match)
            ...
            fetch(employee.avatar.medium)
              .then(image => image.arrayBuffer())
              .then(image => fs.writeFileSync(path.join('public', 'avatars', `${imageName}.jpg`), Buffer.from(image)))
            // Creates an manager with data provided and pushes that manager to the managers array
            managers.push({
              id: employee.id,
              name: employee.name,
              email: employee.email,
        ...
        fs.writeFileSync(path.join('server', 'api', 'managerData.json'), JSON.stringify(managers, null, 2))
        return resolve()
      })
      .catch((err) => {
        return reject(err)
